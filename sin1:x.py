...
class TopologistSineNotArcConnected(Scene):
    def construct(self):
        titre = Text("Courbe sinus du topologue : connexe mais non connexe par arcs", font_size=28)
        sous_titre = Text("Topologist's Sine Curve", font_size=20, color=GREY)
        
        definition_math = MathTex(r"S = \left\{ \left(x, \sin\left(\frac{1}{x}\right)\right) \;\middle|\; x > 0 \right\} \cup \Big\{ (0, y) \;\Big|\; y \in [-1, 1] \Big\}", 
        ...
        axes = Axes(
            ...
            x_length=6,
            y_length=4,
            axis_config={"color": BLUE_D},).to_edge(DOWN, buff=0.8)
        ...
        self.play(graph_group.animate.scale(8, about_point=pt_P.get_center()).shift(-pt_P.get_center()),run_time=3)
        self.wait(1)
        ...
        titre_non_arc = Text("2. Mais est-il Connexe par arcs ?", font_size=24, color=RED).to_edge(DOWN, buff=0.5)
        ...
        conclusion = VGroup(
            ...
            Text("-> Connexe mais NON connexe par arcs", font_size=22, color=RED)
        ...
        final = Text(
            "La connexité par arcs est une condition plus forte que la connexité",
